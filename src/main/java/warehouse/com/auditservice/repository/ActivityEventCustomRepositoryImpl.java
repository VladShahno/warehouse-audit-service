...
public class ActivityEventCustomRepositoryImpl implements ActivityEventCustomRepository {
  ...
  private static final String ACTION_FIELD_PATTERN = Pattern.compile("^[a-zA-Z*?\\-\\s]*$")
      .pattern();
  ...
  public List<ActivityEvent> findAllForCsv(
    ...
    Optional.ofNullable(bodyRequest)
        .ifPresent(brq -> addCriteriaFromBodyActivityEventRequest(criteria, brq));
  ...
  private void addCriteriaFromActivityEventRequest(Criteria criteria,
      ActivityEventRequest request) {
    Optional.ofNullable(request.getEntityId())
        .ifPresent(entityId -> criteria.and(ENTITY_ID_FIELD_NAME).is(entityId));
    Optional.ofNullable(request.getActionSearch())
        .ifPresent(action -> addActionFieldCriteria(criteria, action));
  ...
  private void addCriteriaFromBodyActivityEventRequest(Criteria criteria,
      BodyActivityEventRequest request) {
  ...
  private String getPreparedRegExp(String key) {
    ...
    return MongoRegexCreator.INSTANCE.toRegularExpression(formattedQuery,
        MongoRegexCreator.MatchMode.REGEX);
  }
